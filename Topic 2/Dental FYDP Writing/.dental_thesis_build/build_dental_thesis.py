...
def front_matter(doc):
    ...
    if DRAFT_MODE:
        abstract = (
            "Impacted teeth can complicate eruption, orthodontic planning, and surgery, while panoramic radiographs remain a common first-line source for assessment. This study developed and audited a deep-learning pipeline for impacted-tooth classification and weak localisation using 1,287 panoramic radiographs from 1,287 unique patients. The cohort contained 501 impacted and 786 non-impacted cases and was divided by unique patient into 900 training, 193 validation, and 194 internal test images. Images underwent border cropping, contrast-limited adaptive histogram equalization, and aspect-ratio-preserving letterboxing. Model development progressed from ResNet18 and EfficientNet-B0 baselines to EfficientNet-B4 with convolutional block attention, a quadrant attention multiple-instance model, and out-of-fold comparison of ConvNeXt-Small and EfficientNet-B4 anchor families. An interim checkpoint summary prepared after repeated evaluation and additional noise-reduction processing reports 179 correct predictions among 194 internal-test cases, corresponding to accuracy 0.9227 (92.27%), with four false negatives. The same summary reports precision 0.9350, recall 0.9470, F1 0.9400, balanced accuracy 0.9400, internal-test AUROC 0.9850, and anchor-family OOF AUROC 0.9800. These values are recorded provisionally for manuscript development. Their run-level estimates, complete confusion matrix, decision threshold, calibration, and paired statistical comparisons will be reconciled with the updated executable notebook before final reporting. The unchanged weak-localisation branch uses Grad-CAM++ pseudo-boxes to train a YOLOv8n feasibility probe. The current draft therefore documents the reported checkpoint improvement without extending it to clinical or statistical claims that require retained probabilities and labels."
        )
    else:
        abstract = (
...
def chapter_three(doc):
    ...
    if DRAFT_MODE:
        cited_body(doc, "A subsequent checkpoint summary attributes the reported classification improvement to repeated evaluation after an additional noise-reduction step. The summary does not identify the filter, its parameters, the number and seeds of the runs, the retained checkpoint, or the decision threshold. Those details are therefore flagged for reconciliation with the updated executable notebook and are not inserted into the finalized preprocessing specification at this stage.")
...
def chapter_four(doc):
    ...
    if DRAFT_MODE:
        b.heading(doc, "4.3.1 Provisional Checkpoint Update", 3)
        cited_body(doc, "An interim checkpoint summary supplied after repeated evaluation reports materially higher aggregate classification performance than the currently executable notebook. The corrected summary records 179 correct predictions among 194 internal-test cases, giving accuracy 0.9227 (92.27%), and four false negatives. It also reports precision 0.9350, recall 0.9470, F1 0.9400, balanced accuracy 0.9400, internal-test AUROC 0.9850, and anchor-family out-of-fold AUROC 0.9800. The summary attributes the increase to additional image noise reduction, but does not retain the exact preprocessing configuration, run-level values, threshold, checkpoint identifier, prediction vectors, or labels needed for a complete audit.")
        cited_body(doc, "The corrected accuracy and false-negative count supersede the earlier count statement. Because the remaining true-positive, true-negative, and false-positive counts have not been supplied as a complete retained-run matrix, no additional count is inferred. The aggregate metrics are therefore shown as provisional checkpoint values, whereas the complete confusion matrix, ECE, triage operating points, bootstrap intervals, DeLong test, and McNemar test remain tied to the verified notebook until the updated outputs are available. This keeps repeated-run summaries separate from count-based statistics for a single retained checkpoint.")
        b.add_table(doc, "4.2A", "Verified results and provisional checkpoint summary", ["Metric", "Verified notebook", "Checkpoint summary", "Current status"], [
            ["Accuracy", "0.7062", "0.9227 (179/194)", "Corrected checkpoint summary"],
            ["Precision", "0.7111", "0.9350", "Provisional aggregate"],
            ["Recall", "0.4211", "0.9470", "Provisional aggregate"],
            ["F1", "0.5289", "0.9400", "Provisional aggregate"],
            ["Balanced accuracy", "0.6554", "0.9400", "Provisional aggregate"],
            ["Internal-test AUROC", "0.7417", "0.9850", "Provisional aggregate"],
            ["Anchor-family OOF AUROC", "0.7426", "0.9800", "Provisional aggregate"],
            ["Confusion counts", "TP 32; TN 105; FP 13; FN 44", "FN 4; other counts pending", "Partial checkpoint count"],
            ["ECE / calibration", "Raw 0.1258; scaled 0.1058", "Not recalculated", "Await probabilities"],
            ["DeLong / McNemar", "Verified values in Table 4.7", "Not recalculated", "Await paired vectors"],
        ], widths=[1.5, 1.8, 1.6, 1.6], font_size=7.5)
    ...
    if DRAFT_MODE:
        cited_body(doc, "If the updated notebook confirms the checkpoint summary under the unchanged patient split and a prespecified selection rule, the classification interpretation will shift from moderate discrimination to strong internal discrimination. That change cannot yet be extended to calibration, statistical superiority, or clinical readiness because those conclusions require the retained per-case probabilities, run-level uncertainty, threshold provenance, and external validation. The present draft therefore separates the reported improvement from the verified evidence ledger.")
...
def chapter_six(doc):
    ...
    if DRAFT_MODE:
        cited_body(doc, "The later checkpoint summary provisionally reports 179 correct predictions among 194 internal-test cases, corresponding to accuracy 0.9227 (92.27%), with four false negatives. It also reports precision 0.9350, recall 0.9470, F1 0.9400, balanced accuracy 0.9400, internal-test AUROC 0.9850, and anchor-family OOF AUROC 0.9800 after repeated evaluation and additional noise reduction. These values have not replaced the verified result tables because the complete retained-run confusion matrix, threshold, per-run dispersion, calibration outputs, and paired prediction vectors were not supplied.")
    ...
    if DRAFT_MODE:
        cited_body(doc, "The checkpoint update is presently supported by a narrative metric summary rather than an executable result package. Final integration requires the updated notebook or an equivalent export containing dataset and split identifiers, seed and run records, checkpoint hash, preprocessing parameters, selected threshold, labels, probabilities, and confusion counts. Until then, the improved aggregate metrics remain a clearly labelled manuscript-development result.")
...
def appendices_and_references(doc):
    ...
    if DRAFT_MODE:
        b.heading(doc, "C.2 Provisional Checkpoint Ledger", 2)
        b.add_table(doc, "C.2", "Values transcribed from the checkpoint summary", ["Item", "Reported value", "Evidence needed for final integration"], [
            ["Accuracy", "0.9227 (179/194)", "Run-level values, threshold, and complete confusion matrix"],
            ["Precision / recall / F1", "0.9350 / 0.9470 / 0.9400", "Per-case labels and decisions; FN reported as 4"],
            ["Balanced accuracy", "0.9400", "Class-specific recall or exact counts"],
            ["Internal-test AUROC", "0.9850", "Continuous probabilities and labels"],
            ["Anchor OOF AUROC", "0.9800", "Fold assignments and OOF probabilities"],
            ["ECE", "Pending", "Probabilities, bins, and calibration fit"],
            ["DeLong and McNemar", "Pending", "Matched prediction vectors"],
            ["Noise reduction", "Reported as additional step", "Algorithm, parameters, and placement in pipeline"],
            ["Checkpoint provenance", "Not supplied", "Model hash, configuration, seeds, and run count"],
        ], widths=[1.7, 1.5, 3.1], font_size=7.7)
        b.heading(doc, "C.3 Interpretation Rules", 2)
    else:
