...
def chapter_four(doc):
    ...
    if DRAFT_MODE:
        ...
        cited_body(doc, "An interim checkpoint summary supplied after repeated evaluation reports materially higher aggregate classification performance than the currently executable notebook. The proposed values are accuracy 0.9300, precision 0.9350, recall 0.9470, F1 0.9400, balanced accuracy 0.9400, internal-test AUROC 0.9850, and anchor-family out-of-fold AUROC 0.9800. The summary attributes the increase to additional image noise reduction, but does not retain the exact preprocessing configuration, run-level values, threshold, checkpoint identifier, prediction vectors, or labels needed for a complete audit.")
        cited_body(doc, "The accompanying count statements are not arithmetically compatible with the 194-case test cohort: the reported true-positive, true-negative, false-positive, and false-negative counts sum to 195, while the stated number of correct predictions is given inconsistently. Consequently, the aggregate metrics are shown as provisional checkpoint values, whereas the exact confusion matrix, ECE, triage operating points, bootstrap intervals, DeLong test, and McNemar test remain tied to the verified notebook until the updated outputs are available. This prevents a higher headline accuracy from being combined with incompatible secondary statistics.")
        b.add_table(doc, "4.2A", "Verified results and provisional checkpoint summary", ["Metric", "Verified notebook", "Checkpoint summary", "Current status"], [
            ["Accuracy", "0.7062", "0.9300", "Provisional aggregate"],
            ...
            ["Confusion counts", "TP 32; TN 105; FP 13; FN 44", "Internally inconsistent", "Await updated predictions"],
...
def chapter_six(doc):
    ...
    if DRAFT_MODE:
        cited_body(doc, "The later checkpoint summary provisionally reports accuracy 0.9300, precision 0.9350, recall 0.9470, F1 0.9400, balanced accuracy 0.9400, internal-test AUROC 0.9850, and anchor-family OOF AUROC 0.9800 after repeated evaluation and additional noise reduction. These values have not replaced the verified result tables because the supplied count statements are inconsistent with the 194-case cohort and the threshold, per-run dispersion, calibration outputs, and paired prediction vectors were not supplied.")
...
def appendices_and_references(doc):
    ...
    if DRAFT_MODE:
        ...
        b.add_table(doc, "C.2", "Values transcribed from the checkpoint summary", ["Item", "Reported value", "Evidence needed for final integration"], [
            ["Accuracy", "0.9300", "Run-level values, threshold, and confusion matrix"],
            ["Precision / recall / F1", "0.9350 / 0.9470 / 0.9400", "Per-case labels and decisions"],
