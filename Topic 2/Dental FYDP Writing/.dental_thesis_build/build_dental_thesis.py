...
# The verified 194-case evaluation is the report's current result set.  The
# branch flag is retained only to avoid disturbing the established builder
# structure; the normal build now produces the submission-facing final report.
DRAFT_MODE = True
OUTPUT = WORK / "Dental_FYDP_Final_Report.docx"
...
def chapter_one(doc):
    ...
    b.bullets(doc, [
        ...
        "Document limitations, ethics, reproducibility controls, engineering constraints, and expert-validation requirements.",
    ])
...
def chapter_two(doc):
    ...
    cited_body(doc, "The reference standard is as important as the network architecture. Some studies use consensus among several observers, some rely on clinical categories recorded in practice, and others provide dentist-corrected spatial annotations [11, 22, 23]. This project uses image-level labels reviewed by one doctor and reports that reference structure explicitly. Multi-reader agreement lies outside the present single-reader reference standard.")
    ...
    b.add_table(doc, "2.1", "Synthesis of closely related literature", ["Theme", "Representative work", "Lesson for this project"], [
        ...
        ["Benchmarking", "DENTEX [22]", "Multi-institution hidden testing defines the external benchmark"],
...
def chapter_three(doc):
    ...
    cited_body(doc, f"The dataset contains 1,287 radiographs from 1,287 unique patients, so each patient contributes one image. Image-level impacted/non-impacted labels were reviewed by {VALIDATING_CLINICIAN_PROSE}. This clinical review supports use of the binary labels; multi-reader annotation would additionally allow inter-rater agreement to be estimated. The confirmed reference standard comprises image-level labels; spatial bounding boxes, segmentation masks, tooth-specific categories, and surgical outcomes remain outside this study's reference standard.")
    ...
    cited_body(doc, "The test partition remained isolated from label updates and was retained for final model-family comparison, operating-point reporting, calibration assessment, and error analysis. Because several model variants were eventually compared on this same internal test set, the report describes it as an internal held-out evaluation set. Further model-family decisions require a newly locked external cohort.")
    ...
    b.heading(doc, "3.6 Planned Segmentation Extension", 2)
    ...
    cited_body(doc, "The planned segmentation network follows a U-Net encoder-decoder arrangement. Each encoder stage uses repeated 3 × 3 convolution, batch-normalization, and ReLU operations, with 2 × 2 max-pooling between stages. The decoder restores spatial resolution with 2 × 2 up-convolution and concatenates the corresponding encoder features through skip connections. A final 1 × 1 convolution with sigmoid activation produces the tooth-mask probability map. Under this extension, training would be monitored on the validation partition using Adam optimization and a Dice-based or combined binary-cross-entropy and Dice objective, with model selection based on validation Dice.")
    cited_body(doc, "In the planned extension, a panoramic image would be processed by the validation-selected model to produce a segmentation mask. The workflow includes an image overlay for qualitative inspection. Mask-based evaluation would report Dice coefficient, intersection over union, precision, recall, F1 score, and tooth-type summaries, supplemented by qualitative examples and challenging-case review.")
    cited_body(doc, "Figure 3.2 records this mask-supervised design without treating it as an executed result. The present evidence consists of the completed classification, Grad-CAM++, pseudo-box, and YOLO analyses reported in Chapter 4; expert masks are the prerequisite for evaluating the segmentation extension.")
    b.add_figure(doc, "3.2", "dental_methodology_workflow_supplied.jpg", "Planned segmentation extension showing image-mask loading, preprocessing, training-only augmentation, U-Net encoder-decoder development, validation, inference, visualization, and mask-based evaluation. Chapter 4 reports the completed classification and weak-localisation results.", width=6.05)
...
def chapter_four(doc):
    ...
    if DRAFT_MODE:
        ...
        cited_body(doc, "On the internal test set, the selected model achieved AUROC 0.9850, indicating strong discrimination in the 194-case held-out cohort.")
        cited_body(doc, "The selected model produced 179 correct predictions among 194 cases, corresponding to accuracy 0.9227 (92.27%). The confusion matrix contained 72 true positives, 107 true negatives, 11 false positives, and four false negatives. Precision was 0.8675, recall was 0.9474, specificity was 0.9068, F1 was 0.9057, and balanced accuracy was 0.9271.")
    else:
    ...
    if DRAFT_MODE:
        cited_body(doc, "At the retained decision point, the selected model classified 83 cases as impacted and 111 as non-impacted. It correctly identified 72 of 76 impacted cases and 107 of 118 non-impacted cases. The resulting accuracy was 0.9227, precision was 0.8675, recall was 0.9474, specificity was 0.9068, F1 was 0.9057, and balanced accuracy was 0.9271.")
    ...
    if DRAFT_MODE:
        ...
        cited_body(doc, "These intervals describe uncertainty around the observed internal-test proportions. AUROC remained the primary ranking measure and was 0.9850 for the selected model.")
        ...
        b.add_figure(doc, "4.3", "calibration_and_roc.png", "Internal-test reliability and receiver operating characteristic summary for the selected model (AUROC 0.9850).", width=5.7)
    else:
    ...
    if DRAFT_MODE:
        cited_body(doc, "Grad-CAM++ provided useful inspection artifacts, while expert spatial validation remains a separate evidence requirement. The YOLO metrics identify dentist-defined boxes or masks as the required basis for clinical localisation assessment.")
    else:
...
def chapter_five(doc):
    ...
    cited_body(doc, "The project was organized around transparent medical-imaging AI reporting. CLAIM 2024 guided disclosure of data source, eligibility, reference standard, partitioning, preprocessing, model architecture, selection, evaluation, uncertainty, challenging-case analysis, and availability [24]. The reported evidence is scoped to internal validation and image-level classification; prospective clinical impact and expert spatial validation remain outside that scope.")
    if DRAFT_MODE:
    ...
    cited_body(doc, "Computational sustainability also favors reporting the smallest model that meets a defined need. The strong ResNet18 baseline demonstrates that large backbones are not automatically necessary. Model-family comparisons should include energy, latency, memory, and clinical utility rather than pursuing size alone. Inference measurements of approximately 17.3 ms per image for Phase II and 46.06 ms for MIL provide an initial efficiency record; hardware-specific benchmarking remains an external requirement.")
    ...
    cited_body(doc, "Both authors contributed equally to the project overall through complementary leadership roles. Meherajur Rahman led code architecture, notebook integration, training execution, debugging, and artifact management. Jannatun Nahar led literature synthesis, result interpretation, written presentation, and cross-checking of the clinical and reporting narrative. Both authors jointly contributed to research design, data review, metric selection, discussion, revision, and final approval.")
    ...
    cited_body(doc, "The work qualifies as a complex engineering problem because the requirements are evolving, competing, and clinically constrained. The dataset provides image-level labels, while the intended system also explores localisation. The model must handle heterogeneous radiographs, class imbalance, a 1,287-image unique-patient cohort, sampling uncertainty, and a high cost of false negatives. These tensions require several complementary metrics rather than a single performance measure.")
    if DRAFT_MODE:
...
def chapter_six(doc):
    b.chapter(doc, 6, "Conclusion and Further Validation", "This chapter summarizes the achieved results, clarifies the strength and scope of the evidence, and identifies the validation and data requirements for clinical translation.")
    ...
    if DRAFT_MODE:
        cited_body(doc, "The out-of-fold selector retained the EfficientNet-B4 plus CBAM anchor with AUROC 0.9800. On the internal test set, the selected model achieved AUROC 0.9850 and produced 179 correct predictions among 194 cases, corresponding to accuracy 0.9227 (92.27%). The confusion matrix contained 72 true positives, 107 true negatives, 11 false positives, and four false negatives. Precision was 0.8675, recall was 0.9474, specificity was 0.9068, F1 was 0.9057, and balanced accuracy was 0.9271.")
    else:
    ...
    cited_body(doc, "The study's evidence is bounded to a 1,287-image unique-patient cohort, including a 194-image internal test set, and a single-clinician-validated binary reference. This design supports reproducible internal development and evaluation. Generalization across institutions, devices, demographic groups, and clinically relevant subgroups requires a separately locked external cohort and corresponding metadata. Because the internal test set also supported descriptive model comparisons, it is not used to justify further architecture selection.")
    if DRAFT_MODE:
    ...
    b.heading(doc, "6.4 Further Validation and Development", 2)
    ...
    cited_body(doc, "Deep learning can support systematic analysis of impacted teeth on panoramic radiographs when performance is interpreted within its evidence scope. The present work completes a transparent internal-development study, quantifies its operating characteristics, and separates verified classification evidence from exploratory weak localisation. Expert spatial annotation, external validation, and clinically grounded evaluation are the defined requirements for translation beyond this scope.")
...
def appendices_and_references(doc):
    ...
    b.add_table(doc, "F.1", "How the reviewed literature informed the project", ["Project decision", "Supporting literature", "Adaptation in this study"], [
        ...
        ["External benchmark planning", "DENTEX [22]", "Dentist-verified spatial and external evaluation"],
