...
% not imported or modified. This file is a self-contained journal-specific draft
...
% Keep the draft visually clean while retaining page numbers.
\ExplSyntaxOn
...
text-prior product-of-experts variant, an image-noise variant, an exploratory
...
archived implementation; independent labels, matched controls, corrected
...
text-only and image-only controls. Figure~\ref{fig:design} connects cohort
provenance, the model pathways, controlled ablations, and seed-level inference.

\begin{figure}[pos=p]
\centering
\textbf{(a)}\par\smallskip
\includegraphics[width=0.98\linewidth]{methodology.png}\par\medskip
\textbf{(b)}\par\smallskip
\includegraphics[width=0.50\linewidth]{cohort_flow.png}
\caption{Study design. (a) Methodological framework linking data provenance,
paired image-report inputs, controlled ablations, and evaluation. The audit
compares five fusion conditions over eight seeds and separates predictive
performance from incremental image contribution. (b) Cohort-construction flow.
Nine unavailable JPG files were removed before the final 2,953-study cohort was
partitioned at patient level.}
\label{fig:design}
\end{figure}
...
fixed seed. Baseline CADQ, LearnedMixinFusion (LMH), Noise-Consistency,
GACR-B, and Adaptive-CADQ each had eight retained seeds: 42, 123, 456, 789,
1010, 1111, 1212, and 1313. The eight-seed plan covered baseline, LMH, and
Noise-Consistency; Adaptive-CADQ was extended from three to eight seeds after
the initial run, and GACR-B was added post hoc. The latter two analyses are
...
implementation limitation requiring a corrected rerun
...
\cite{huang2017densenet,cohen2022torchxrayvision}. Its convolutional stem and
...
unimodal-bias reduction \cite{cadene2019rubi}. Noise-Consistency perturbed the
...
divergence; throughout this manuscript it is treated as an anti-consistency or
noise-sensitivity condition, not a valid conventional consistency objective.
...
a cosine penalty of 0.3 on examples for which that baseline was wrong, following
...
conditions. The archived image-only pipeline did not consistently restore its
best checkpoint and is reported as descriptive.
...
($0.9849\pm0.0113$). Noise-Consistency had the lowest Macro-F1
($0.9346\pm0.0075$). Exploratory GACR-B reached $0.9417\pm0.0059$, and
...
deviation). GACR-B was added post hoc and is exploratory.}
...
Noise-Consistency & $0.9346\pm0.0075$ & $0.9560\pm0.0039$ & $0.9960\pm0.0013$ & $0.0347\pm0.0137$ & $0.0655\pm0.0130$ \\
GACR-B (exploratory) & $0.9417\pm0.0059$ & $0.9610\pm0.0042$ & $0.9972\pm0.0005$ & $0.0245\pm0.0023$ & $0.0540\pm0.0038$ \\
...
reduced Pneumonia F1 to $0.9575\pm0.0077$. Noise-Consistency had the
...
(b) Retained single-run controls compared with the baseline VLM. Image-only
checkpoint selection was imperfect, and control values are descriptive rather
...
adjusted $p=0.2188$). LMH and Noise-Consistency had lower seed-42
...
Noise-Consistency & 0.1953 (0.3333) & 0.0022 (0.0044) & 32/49; 0.0754 (0.3018) \\
GACR-B (exploratory) & 0.0547 (0.2188) & 0.0315 (0.0420) & 9/8; 1.0000 (1.0000) \\
Adaptive-CADQ & 0.8438 (0.8438) & 0.7304 (0.7304) & 15/14; 1.0000 (1.0000) \\
\bottomrule
\end{tabular}}
\end{table}

The balanced discordance for GACR-B and Adaptive-CADQ is informative but less
extreme than the stale interpretation in the original draft. GACR-B disagreed
...
Noise-Consistency, and GACR-B reflect lower seed-42 Pneumothorax AUROC than
...
Noise-Consistency increased Brier score to $0.0655\pm0.0130$ (adjusted
$p=0.0156$), whereas its ECE change was inconclusive. GACR-B and
...
image-only checkpoint was not selected consistently, they support an audit of
...
not meet the equivalence criterion. Noise-Consistency had a negative mean ICM
($-0.0039\pm0.0081$), whereas exploratory GACR-B and Adaptive-CADQ had small
...
\centering
\textbf{(a)}\par\smallskip
\includegraphics[width=0.55\linewidth]{paired_vlm_gray_square.png}\par\medskip
\textbf{(b)}\par\smallskip
\includegraphics[width=0.55\linewidth]{icm_tost_master.png}\par\medskip
\textbf{(c)}\par\smallskip
\includegraphics[width=0.55\linewidth]{gate_stagnation_with_dots.png}
...
clinically wrong, that every prediction copied a particular phrase, or that
...
LMH. Noise-Consistency cannot support its intended interpretation because the
archived negative-KL sign rewards divergence. Adaptive-CADQ changed the gate
form but not the endpoint. Exploratory GACR-B, following the grounding-aware
...
The corrected McNemar interpretation is also important. GACR-B and
...
Implementation defects limit causal interpretation. Image values were mapped
to $[-1,1]$, which may not match the native TorchXRayVision convention for the
selected weights. The Noise-Consistency objective had the wrong KL sign. The
attention block used one token per modality. Image-only checkpoint restoration
was inconsistent. These defects cannot be repaired by manuscript wording; the
affected experiments require rerunning. The current findings audit the archived
system and should not be presented as a definitive test of correctly
implemented noise consistency or optimized image-only performance.
...
GACR-B were post hoc. No a priori sample-size calculation established power for
...
labels, corrected implementations, matched multi-seed controls, provenance
...
Noise-Consistency & 8 seeds & Metrics, predictions, probabilities & Negative KL sign rewards divergence \\
GACR-B & 8 seeds & Metrics, predictions, recovered probabilities & Added post hoc; merge provenance was not hash-locked \\
Adaptive-CADQ & 8 seeds & Metrics, predictions, probabilities & Extended from three to eight seeds post hoc \\
Controls & One retained run each & Metrics and selected arrays & Gray-Square uncertainty not replicated; image-only best-checkpoint restoration inconsistent \\
\bottomrule
\end{tabular}
\end{table}
...
Noise-Consistency & 0.962 [0.929, 0.980] & 0.947 [0.903, 0.972] & 0.894 [0.774, 0.954] \\
GACR-B & 0.975 [0.946, 0.988] & 0.947 [0.903, 0.972] & 0.894 [0.774, 0.954] \\
