...
but less extreme than the stale interpretation in the original draft.
...
image-only checkpoint was not selected uniformly, they support an audit of
...
used one token per modality. Image-only checkpoint restoration was not uniform.
These limitations cannot be resolved by manuscript wording; the affected
experiments need confirmatory reanalysis. The current findings audit the
archived system and should not be presented as a definitive evaluation of
correctly implemented noise-sensitivity (anti-consistency) or optimized
