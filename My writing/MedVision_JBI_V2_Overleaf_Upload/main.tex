...
\includegraphics[width=\linewidth]{methodology_diagram_v3_abc.png}\par\medskip
\textbf{(d)}\par\smallskip
\includegraphics[height=0.35\textheight,keepaspectratio]{cohort_flow.png}
\caption{Study design and audit workflow. (a) DenseNet-121 image pathway.
(b) Bio\_ClinicalBERT report pathway. (c) Cross-attention, per-class CADQ
fusion, five-condition eight-seed audit, ICM, retained-gate inspection, and
controls. The architecture panel is a conceptual overview; the archived model
used 512-dimensional projections, findings-only text, separate class logits,
and a static per-class baseline gate. Its arrows should not be read as a
layer-by-layer computational graph. The permutation icon is illustrative only;
DeLong and McNemar use retained seed-42 outputs, and each control has one
retained run. (d) Retained
cohort flow: nine unavailable JPG files were removed before the final
2,953-study cohort was partitioned at patient level.}
\label{fig:design}
\end{figure}
...
\begin{figure}[pos=H]
\centering
\includegraphics[width=0.46\linewidth]{gradcam_baseline.png}
...
\renewcommand{\bibfont}{\fontsize{6pt}{6.6pt}\selectfont}
\begingroup
\sloppy
